...
public class UssTestHelpers {
    ...
    public static LoginService createLoginService(Optional<GenericDao<String, User>> maybeDao) {
        var dao = maybeDao.orElseGet(UssTestHelpers::createUserDao);
        return new LoginService(dao);
    }

    public static SignUpService createSignUpService(Optional<GenericDao<String, User>> maybeDao) {
        var dao = maybeDao.orElseGet(UssTestHelpers::createUserDao);
        return new SignUpService(dao, new MailSenderListImpl());
    }
